...
class ConvertPath(TemplateView):
    ...
    def maxim_format_pathway(self, filepath, organism, database):
        """
    ...
    def convert_human_pathway_to_mouse(self, database, delete_old):
        """
        Function to convert Human Pathways to Mouse. 
        For the whole database at once
        
        delete_old - trigger to delete old pathways from corresponding mouse database
        
        """
        
        if delete_old:
            for mouse_path in Pathway.objects.filter(organism='mouse', database=database):
                mouse_path.delete()
        
        human_paths = Pathway.objects.filter(organism='human', database=database)
        i=0
        for human_path in human_paths:
            i=i+1
            print human_path.name+' i = '+str(i) #to see progress in console
            
            try:
                mouse_path = Pathway.objects.get(organism='mouse', database=database, name=human_path.name)
            except:
                mouse_path = Pathway(organism='mouse', database=database, name=human_path.name, amcf=human_path.amcf)
                mouse_path.save()
            
            
            #Add genes
            for human_gene in human_path.gene_set.all():
                try:
                    mapping = MouseMapping.objects.filter(human_gene_symbol=human_gene.name)[0] #get first occurrence
                
                    mouse_gene = Gene(name=mapping.mouse_gene_symbol.upper(), #make gene name uppercase 
                                       arr=human_gene.arr, pathway=mouse_path,
                                       comment=human_gene.comment)
                except: # if no match found in Mapping table
                    mouse_gene = Gene(name=human_gene.name, arr=human_gene.arr, pathway=mouse_path,
                               comment=human_gene.comment)
                    
                mouse_gene.save()
            
            #Add Nodes and Components    
            for human_node in human_path.node_set.all():
                mouse_node = Node(name=human_node.name, comment=human_node.comment, pathway=mouse_path)
                mouse_node.save()
                
                for human_component in human_node.component_set.all():
                    try:
                        mapping = MouseMapping.objects.filter(human_gene_symbol=human_component.name)[0]
                        mouse_component = Component(name=human_component.mouse_gene_symbol.upper(), node=mouse_node)
                        
                    except:
                        mouse_component = Component(name=human_node.name, node=mouse_node)
                    mouse_component.save()
                    
            #Add relations
            for human_node in human_path.node_set.all():
                for human_inrel in human_node.inrelations.all():
                    
                    mouse_from_node = Node.objects.filter(name=human_inrel.fromnode.name, pathway=mouse_path)[0]
                    mouse_to_node = Node.objects.filter(name=human_inrel.tonode.name, pathway=mouse_path)[0]
                    
                    mouse_relation = Relation(fromnode=mouse_from_node,
                                               tonode=mouse_to_node,
                                               reltype = human_inrel.reltype)
                    mouse_relation.save()    
