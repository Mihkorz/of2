                ...
                if is_metabolic=='true':
                    try:
                        ...
                        df_1 = pd.read_csv(settings.MEDIA_ROOT+'/report-portal/'+report.slug+'/Extended/New_top_model_FC_whole.csv',
                                  index_col='probeset')
                    try:
                        df_2 = pd.read_csv(settings.MEDIA_ROOT+'/report-portal/'+report.slug+'/Extended/New_top_model_FC_'+lMembers[1]+'.csv',
                                  index_col='probeset')                        
                    except:
                        df_2 = pd.read_csv(settings.MEDIA_ROOT+'/report-portal/'+report.slug+'/Extended/New_top_model_FC_whole.csv',
                                  index_col='probeset')
                    ...
                    try:
                        ...
                        df_1 = pd.read_csv(settings.MEDIA_ROOT+'/report-portal/'+report.slug+'/Extended/New_top_model_FC_ext_whole.csv',
                                  index_col='probeset')
                    try:
                        df_2 = pd.read_csv(settings.MEDIA_ROOT+'/report-portal/'+report.slug+'/Extended/New_top_model_FC_ext_'+lMembers[1]+'.csv',
                                  index_col='probeset')
                    except:
                        df_2 = pd.read_csv(settings.MEDIA_ROOT+'/report-portal/'+report.slug+'/Extended/New_top_model_FC_ext_whole.csv',
                                  index_col='probeset')
...
class ReportDlFarmJson(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if lGname[0]!='overall':
            ...
            try:
                df_farm = df_farm[(df_farm['compound']== lGname[0]) & 
                           ...
                           (df_farm['time']==lGname[2]) &
                           (df_farm['celltype']==lGname[1] )]
            except:
                df_farm = df_farm[(df_farm['compound']== lGname[0]+lGname[1]) & 
        ...
        if file_type == 'fc':
            s_features = df_farm.iloc[:, 9:] # leave only columns with features, excluding 'Best threshold'
        if file_type == 'se':
