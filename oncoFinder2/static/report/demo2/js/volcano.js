function drawVolcanoPlot(renderTo,  file_name){
	...
	// making the plot more readable.
	    axesMargin = 10,
	    width = 800 - margin.left - margin.right - axesMargin,
	    height = 400 - margin.top - margin.bottom - axesMargin;
	...
	d3.json('/report-portal/demo2-genevolcanojson/')
	...
	//d3.json("/static/report/bc/GSE55633_gene_expressions_de.json", function (error, root) {	
	 ...
	 // Let's create a group to contain all the data points, correctly spaced from the axes.
	    ...
	    var circles = scatter.selectAll("circle").data(root, getSymbol)
	        .enter().append("circle")
	 ...
	 // Let's create a Voronoi diagram using the accessors defined before.
	    ...
	    circles.each(function(d) {
	        var content = "<p>Log2 Fold Changes: " + getX(d) + "<br>" +
	            "Adjusted P-Value: " + Math.pow(10, -1*getY(d)) + "</p>";
